...
class TestGitHubCLI:
    """Legacy tests converted to use GitHubClient class (no longer skipped)."""
    
    def test_gh_issue_view(self, claude_tasker_script, mock_git_repo):
        """Test GitHub issue viewing - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_get_issue_success
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        # Simple validation that the class exists and has the method
        assert hasattr(client, 'get_issue'), "GitHubClient should have get_issue method"
    
    def test_gh_pr_view(self, claude_tasker_script, mock_git_repo):
        """Test GitHub PR viewing - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_get_pr_success  
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, 'get_pr'), "GitHubClient should have get_pr method"
    
    def test_gh_pr_diff(self, claude_tasker_script, mock_git_repo):
        """Test GitHub PR diff - delegates to GitHubClient."""
        # This functionality is tested via get_pr_diff in TestGitHubClient
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, 'get_pr_diff'), "GitHubClient should have get_pr_diff method"
    
    def test_gh_pr_create(self, claude_tasker_script, mock_git_repo):
        """Test GitHub PR creation - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_create_pr_success
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, 'create_pr'), "GitHubClient should have create_pr method"
    
    def test_gh_issue_comment(self, claude_tasker_script, mock_git_repo):
        """Test GitHub issue commenting - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_comment_on_issue_success
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, 'comment_on_issue'), "GitHubClient should have comment_on_issue method"
    
    def test_gh_pr_list_search(self, claude_tasker_script, mock_git_repo):
        """Test GitHub PR search - delegates to GitHubClient."""
        # This functionality is tested via _run_gh_command in TestGitHubClient
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, '_run_gh_command'), "GitHubClient should have _run_gh_command method"
    
    def test_gh_project_view(self, claude_tasker_script, mock_git_repo):
        """Test GitHub project viewing - delegates to GitHubClient."""
        # This functionality is tested via get_project_info in TestGitHubClient
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, 'get_project_info'), "GitHubClient should have get_project_info method"
    
    def test_gh_api_calls(self, claude_tasker_script, mock_git_repo):
        """Test GitHub API calls - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_run_gh_command_success
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, '_run_gh_command'), "GitHubClient should have _run_gh_command method"
    
    def test_gh_error_handling(self, claude_tasker_script, mock_git_repo):
        """Test GitHub error handling - delegates to GitHubClient."""
        # This functionality is fully tested in TestGitHubClient.test_run_gh_command_failure
        from src.claude_tasker.github_client import GitHubClient
        client = GitHubClient()
        assert hasattr(client, '_run_gh_command'), "GitHubClient should have _run_gh_command method"
